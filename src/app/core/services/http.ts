...
export class HttpService extends Http {
   ...
   * @returns {Observable<>}
   */
  post3rdParty(thirdparty: string, url: string, body: any): Observable<any> {
    this.requestInterceptor();
    return super.post(this.get3rdPartyUrl(thirdparty, url),
      body, this.get3rdPartyRequestOptions(thirdparty))
      .catch(this.onCatch.bind(this))
      .do((res: Response) => {
        this.onSubscribeSuccess(res);
      }, (error: any) => {
  ...
  /**
   * Build 3rd party API url.
   * @param url
   * @returns {string}
   */
  private get3rdPartyUrl(thirdparty: string, url: string): string {
    let fullUrl: string;
    switch (thirdparty) {
      case 'paymaya': 
        fullUrl = environment.PAYMAYA.API + url;
        break;
      default:
        fullUrl = url;
    }
  ...
  /**
   * 3rd Party request options.
   * @param options
   * @returns {RequestOptionsArgs}
   */
  private get3rdPartyRequestOptions(thirdparty: string): RequestOptionsArgs {
    const options = new RequestOptions();
    switch (thirdparty) {
      case 'paymaya':
        options.headers = new Headers({
          'Content-Type': 'application/json',
          'Authorization': `Basic ${environment.PAYMAYA.PUBLIC_KEY}`,
        });
        break;
      default:
        // const token = localStorage.getItem('omg_token');
